...
export const uploadClientDocumentController = async (req, res) => {
    try {
        ...
        if (singleInstanceTypes.includes(documentType)) {
            ...
            // Check if documentType starts with "MSME Certificate" to allow updates for specific MSME rows
            // or if it's a generic "Other" type that we want to append.
            // For MSME, the frontend sends "MSME Certificate - <UdyamNumber>" or similar if we wanted unique.
            // But usually it sends "MSME Certificate" and we rely on the row index.
            // Actually, the frontend CompanyDocument sends "MSME Certificate" as type.
            // Since there can be multiple MSME certificates (one per row), we should always PUSH new ones
            // OR update if we can identify them.
            // For now, consistent with "Always push" for multiple-instance types.
...
export const deleteClientDocumentController = async (req, res) => {
    try {
        ...
        // We can optionally check if doc exists first to return 404, or just pull.
        // If we want to return 404 if doc not found, we can query it.
...
export const updatePlantProcessProgressController = async (req, res) => {
    try {
        const { clientId } = req.params;
        ...
        if (!type || !itemId || !completedSteps) {
            return res.status(400).json({
                message: "Missing required fields: type, itemId, completedSteps",
                error: true,
                success: false
            });
        }
