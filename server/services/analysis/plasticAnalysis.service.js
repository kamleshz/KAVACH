...
class PlasticAnalysisService {
   ...
   */
  static async runPlasticAnalysis(
    salesFilePath,
    purchaseFilePath,
    outputDir,
    context = {},
    ...
    const result = await this.calculatePrePost(
      salesFilePath,
      purchaseFilePath,
      outputDir,
  ...
  static async getPlasticAnalysis(clientId, type, itemId) {
    ...
    const doc = await PlasticAnalysisModel.findOne({
      client: clientId,
      type,
      itemId,
    });
  ...
  static async saveAnalysisResult(clientId, type, itemId, result) {
    ...
    if (clientExists) {
      let doc = await PlasticAnalysisModel.findOne({
        client: clientId,
        type,
        itemId,
      });
  ...
  static async calculatePrePost(
    ...
    if (!fs.existsSync(salesFilePath))
      throw new Error("Sales/Base file not found");
    ...
    const salesDataRaw = XLSX.utils.sheet_to_json(salesSheet, {
      defval: 0,
      raw: false,
    });
    ...
    salesData.forEach((row) => {
      ...
      if (cat) {
        const pre =
          parseFloat(row["Pre Consumer Waste Plastic Quantity (TPA)"]) || 0;
        const post =
          parseFloat(row["Post Consumer Waste Plastic Quantity (TPA)"]) || 0;
        const exp =
          parseFloat(row["Export Quantity Plastic Quantity (TPA)"]) || 0;
        ...
        if (year) {
          ...
          const preRec =
            parseFloat(row["Pre Consumer Waste Recycled Plastic %"]) || 0;
          const postRec =
            parseFloat(row["Post Consumer Waste Recycled Plastic %"]) || 0;
          const recycledSum = preRec + postRec;

          if (!recycledPctAgg[cat]) recycledPctAgg[cat] = {};
          recycledPctAgg[cat][year] =
            (recycledPctAgg[cat][year] || 0) + recycledSum;

          // Recycled content achieved (MT) for the year, based on recycled % and quantities
          const expRec =
            parseFloat(
              row["Export Quantity Recycled Plastic %"] ??
                row["Export Quantity Plastic Recycled %"] ??
                row["Export Quantity Recycled Plastic Percentage"] ??
                0,
            ) || 0;
          const achieved = (pre * preRec + post * postRec + exp * expRec) / 100;
          if (!recycledAchievedAgg[cat]) recycledAchievedAgg[cat] = {};
          recycledAchievedAgg[cat][year] =
            (recycledAchievedAgg[cat][year] || 0) + achieved;
        }
      }
    });
    ...
    const years = Array.from(
      new Set(salesData.map((r) => r["Year"]).filter((y) => y)),
    ).sort();
    const targetTables = [];
    ...
    if (clientId && type && itemId) {
      salesAnalysisDoc = await PlasticAnalysisModel.findOne({
        client: clientId,
        type,
        itemId,
      });
    ...
    const getRegisteredSales = (cat, year) => {
      ...
      const rows = salesAnalysisDoc.salesRows.filter((r) => {
        ...
        return (
          rCat === targetCat &&
          rYear === year &&
          rType.includes("registered") &&
          !rType.includes("unregistered") &&
          statusOk
        );
      });

      return rows.reduce(
        (sum, r) => sum + (parseFloat(r.totalPlasticQty) || 0),
        0,
      );
    };
    ...
    if (years.length >= 2) {
      for (let i = 0; i < years.length - 1; i++) {
        ...
        const tableData = Object.keys(salesAgg).map((cat) => {
          ...
          if (isProducer) {
            row[`Registered Sales (${year2})`] = parseFloat(
              registeredSales.toFixed(4),
            );
            row[`Target Of Virgin ${targetYear}`] = parseFloat(
              targetVal.toFixed(4),
            );
          } else {
            ...
            row[`Target For Virgin (${targetYear})`] = parseFloat(
              targetForVirgin.toFixed(4),
            );
          }
          return row;
        });
    ...
    if (!isProducer && years.length > 0) {
      ...
      const urepData = categories.map((cat) => {
        ...
        const pct =
          catTargets[activeYear] !== undefined ? catTargets[activeYear] : 0;
        const baseQty = parseFloat(
          (yearlyAgg[cat] && yearlyAgg[cat][activeYear]) || 0,
        );
    ...
    const normalizePurchaseFinancialYear = (value) => {
    ...
    const getPurchaseFinancialYear = (row) => {
      if (!row) return null;
      ...
      const candidateKeys = ["financial year", "financialyear", "fy", "year"];
      ...
      if (!sourceKey) {
        sourceKey =
          keys.find((k) => {
            ...
            return lk.includes("financial") && lk.includes("year");
          }) ||
          keys.find((k) => k.toLowerCase() === "fy") ||
    ...
    if (hasPurchaseFile) {
      ...
      purchaseData.forEach((row) => {
        ...
        const fy = normalizePurchaseFinancialYear(
          getPurchaseFinancialYear(row),
        );
        if (fy) {
          if (!purchaseAggByYear[fy]) purchaseAggByYear[fy] = {};
          purchaseAggByYear[fy][cat] =
            (purchaseAggByYear[fy][cat] || 0) + (qty || 0);
        }
      ...
      try {
        ...
        if (!analysisDoc) {
          analysisDoc = await PlasticAnalysisModel.findOne({
            client: clientId,
            type,
            itemId,
          });
        ...
        if (
          analysisDoc &&
          analysisDoc.purchaseRows &&
          analysisDoc.purchaseRows.length > 0
        ) {
          analysisDoc.purchaseRows.forEach((row) => {
            const catRaw =
              row.plasticCategory || row.category || row["Category of Plastic"];
            const cat = normalizePurchaseCategory(catRaw);
            if (!cat) return;
    ...
    if (totalRow["Total Purchase"] > 0) {
      totalRow["Difference (%)"] =
        (totalRow["Total Consumption"] / totalRow["Total Purchase"] - 1) * 100;
    }
    ...
    totalRow["Difference (%)"] = parseFloat(
      totalRow["Difference (%)"].toFixed(2),
    );

    finalData.push(totalRow);

    const recycledAchievedByYearCategory = Object.keys(
      recycledAchievedAgg || {},
    ).reduce((acc, cat) => {
      const byYear = recycledAchievedAgg[cat] || {};
      Object.keys(byYear).forEach((year) => {
        if (!acc[year]) acc[year] = {};
        acc[year][cat] = parseFloat(Number(byYear[year] || 0).toFixed(4));
      });
      return acc;
    }, {});

    const purchaseByYearCategory = Object.keys(purchaseAggByYear || {}).reduce(
      (acc, year) => {
        acc[year] = {};
        const bucket = purchaseAggByYear[year] || {};
        Object.keys(bucket).forEach((cat) => {
          acc[year][cat] = parseFloat(Number(bucket[cat] || 0).toFixed(4));
        });
        return acc;
      },
      {},
    ...
    const brandOwnerRecycledContentTable = (() => {
      if (isProducer) return null;
      const yearsInMatrix = ["2025-26", "2026-27", "2027-28", "2028-29"];
      const targets = {
        "Cat-I": { "2025-26": 30, "2026-27": 40, "2027-28": 50, "2028-29": 60 },
        "Cat-II": {
          "2025-26": 10,
          "2026-27": 10,
          "2027-28": 20,
      ...
      const carryForwardBaseByCat = {};
      const minCarryByCat = {};
      const carryForwardByYearCat = {};
      const totalByYearCat = {};

      categories.forEach((cat) => {
        const pct = targets[cat]?.["2025-26"] ?? 0;
        const purchaseQty = purchaseByYearCategory?.["2025-26"]?.[cat];
        const achieved = recycledAchievedByYearCategory?.["2025-26"]?.[cat];
        const obligation =
          typeof purchaseQty === "number" ? (purchaseQty * pct) / 100 : null;
        const baseCarry =
          typeof obligation === "number" && typeof achieved === "number"
            ? Math.max(0, obligation - achieved)
            : 0;
        carryForwardBaseByCat[cat] = baseCarry;
        const safeBaseCarry = typeof baseCarry === "number" ? baseCarry : 0;
        minCarryByCat[cat] = safeBaseCarry / 3;

        carryForwardByYearCat[cat] = { "2025-26": baseCarry };
        totalByYearCat[cat] = { "2025-26": baseCarry };

        let prevTotal = baseCarry;
        yearsInMatrix.slice(1).forEach((year) => {
          const targetPct = targets[cat]?.[year] ?? 0;
          const pQty = purchaseByYearCategory?.[year]?.[cat];
          const ach = recycledAchievedByYearCategory?.[year]?.[cat];
          const obl =
            typeof pQty === "number" ? (pQty * targetPct) / 100 : null;
          const toBeAbs =
            typeof obl === "number" && typeof ach === "number"
              ? Math.abs(obl - ach)
              : 0;
          const carryForward = prevTotal;
          const total =
            year === "2026-27"
              ? toBeAbs + (minCarryByCat[cat] || 0)
              : toBeAbs + carryForward + (minCarryByCat[cat] || 0);
          carryForwardByYearCat[cat][year] = carryForward;
          totalByYearCat[cat][year] = total;
          prevTotal = total;
        });
      });

      const rows = [];
      yearsInMatrix.forEach((year) => {
        categories.forEach((cat) => {
          const applicableTarget = targets[cat]?.[year] ?? 0;
          const purchaseQty = purchaseByYearCategory?.[year]?.[cat];
          const achieved = recycledAchievedByYearCategory?.[year]?.[cat];
          const obligation =
            typeof purchaseQty === "number"
              ? (purchaseQty * applicableTarget) / 100
              : null;
          const toBeAbs =
            typeof obligation === "number" && typeof achieved === "number"
          ...
          const carryForwardBalance =
            year === "2025-26"
              ? carryForwardBaseByCat[cat]
              : (carryForwardByYearCat?.[cat]?.[year] ?? null);
          const minimumCarryForward =
            year === "2025-26" ? null : (minCarryByCat[cat] ?? null);
          const totalRecycledContent =
            year === "2025-26"
              ? carryForwardBaseByCat[cat]
              : (totalByYearCat?.[cat]?.[year] ?? null);

          rows.push({
            financialYear: year,
            category: cat,
            applicableTargetPct: applicableTarget,
            purchaseQty:
              typeof purchaseQty === "number"
                ? parseFloat(purchaseQty.toFixed(3))
                : null,
            obligationBasedTarget:
              typeof obligation === "number"
                ? parseFloat(obligation.toFixed(2))
                : null,
            actualAchieved:
              typeof achieved === "number"
                ...
                : null,
            toBeAchieved:
              typeof toBeAbs === "number"
                ? parseFloat(toBeAbs.toFixed(2))
                : null,
            carryForwardBalance:
              typeof carryForwardBalance === "number"
                ? parseFloat(carryForwardBalance.toFixed(2))
                : null,
            minimumCarryForward:
              typeof minimumCarryForward === "number"
                ? parseFloat(minimumCarryForward.toFixed(2))
                : null,
            totalRecycledContent:
              typeof totalRecycledContent === "number"
                ? parseFloat(totalRecycledContent.toFixed(2))
                : null,
          });
        });
      });
    ...
    return {
      ...
      summary: {
        ...
        purchase_by_year_category: purchaseByYearCategory,
        recycled_achieved_by_year_category: recycledAchievedByYearCategory,
        brand_owner_recycled_content_table: brandOwnerRecycledContentTable,
      },
    };
  }
   ...
   */
  static generateAuditorInsights(
    salesSummary,
    purchaseSummary,
    prePostSummary,
    targetTables,
    ...
    const highDiffCats = prePostSummary.filter(
      (row) => Math.abs(row.difference) > 10,
    );
    if (highDiffCats.length > 0) {
      const cats = highDiffCats.map((c) => c.category).join(", ");
      insights.validation.push(
        `Critical discrepancies (>10%) detected in: ${cats}. Immediate reconciliation of inventory logs and process loss data is recommended.`,
      );
    } else {
      insights.validation.push(
        "All categories are within the acceptable 10% variance threshold. Consumption data aligns well with procurement records.",
      );
    }

    const totalPurchase = prePostSummary.reduce(
      (sum, r) => sum + (parseFloat(r.purchase) || 0),
      0,
    );
    const totalConsumption = prePostSummary.reduce(
    ...
    if (totalPurchase > 0) {
      ...
      insights.validation.push(
        `Overall mass balance shows a ${Math.abs(diff).toFixed(2)}% ${diff > 0 ? "surplus" : "deficit"} (Purchase: ${totalPurchase.toFixed(2)} MT vs Consumption: ${totalConsumption.toFixed(2)} MT).`,
      );
    }
    ...
    if (targetTables.length > 0) {
      ...
      insights.targets.push(
        `Projected EPR obligation for ${targetYear} is approximately ${totalTarget.toFixed(2)} MT based on current averages.`,
      );
      insights.targets.push(
        "Targets are calculated using the average of the last two financial years, adjusted for registered sales (if applicable for Producers).",
      );
    }
    ...
    if (salesTotalRow) {
      ...
      if (total > 0) {
        ...
        insights.sales.push(
          `Total Sales Volume: ${total.toFixed(2)} MT. Registered Sales account for ${regPct}% of total volume.`,
        );
        if (parseFloat(regPct) < 50) {
          insights.sales.push(
            "High volume of unregistered sales detected. Recommendation: Verify invoicing protocols for unregistered buyers.",
          );
        }
      }
    }
    ...
    // 4. Purchase Insights
    const purchaseTotalRow = purchaseSummary.data.find(
      (r) => r.category === "Total",
    );
    if (purchaseTotalRow) {
      ...
      if (total > 0) {
        ...
        insights.purchase.push(
          `Total Procurement: ${total.toFixed(2)} MT. Unregistered procurement stands at ${unregPct}%.`,
        );
        if (parseFloat(unregPct) > 20) {
          insights.purchase.push(
            "Significant reliance on unregistered suppliers. Risk of non-compliance with sourcing norms.",
          );
        }
      }
    }
