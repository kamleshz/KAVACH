...
const SummaryReport = ({
  ...
  const tableData = useMemo(() => {
    ...
    return Array.from(groupedBySku.values()).map((group, index) => {
      ...
      componentCodes.forEach((compCode) => {
        ...
        if (procurementRecords.length > 0) {
          ...
          details.push({
            ...
            recycledQty:
              totalRecycledQtyForComp > 0
                ? totalRecycledQtyForComp.toFixed(3)
                : "0",
            recycledAmount: "0",
            virginQty: "0",
            virginAmount: "0",
      ...
      const totalRecycledQty = details.reduce(
        (sum, item) => sum + (parseFloat(item.recycledQty) || 0),
        0,
      );
      const totalRecycledAmount = details.reduce(
        (sum, item) => sum + (parseFloat(item.recycledAmount) || 0),
        0,
      );
      const totalVirginQty = details.reduce(
        (sum, item) => sum + (parseFloat(item.virginQty) || 0),
        0,
      );
      const totalVirginAmount = details.reduce(
        (sum, item) => sum + (parseFloat(item.virginAmount) || 0),
        0,
      );
      const totalPurchaseMt = details.reduce(
        (sum, item) => sum + (parseFloat(item.monthlyPurchaseMt) || 0),
        0,
      );
      ...
      const hasAnyStatus = details.some(
        (d) =>
          ...
          d.componentComplianceStatus !== "Select",
      ...
      return {
        ...
        computedRemarks: details
          .map((d) => d.auditorRemarks)
          .filter(Boolean)
          .join("\n"),
        ...
        onlyTable ? (
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const skuGroup = tableData.find(
                (g) => g.skuCode === record.skuCode,
              );
              if (skuGroup && skuGroup.details) {
                ...
                const newProductStatus = isAnyNonCompliant
                  ? "Non-Compliant"
                  : hasAnyStatus
                    ? "Compliant"
                    : "";
    ...
    },
    {
      ...
      render: (_, record) => {
        const isThisRowSaving =
          ...
          (isProducer
            ? (productRows[savingRow].componentCode || "").trim() ===
              record.componentCode
            : (productRows[savingRow].skuCode || "").trim() ===
                record.skuCode &&
              (productRows[savingRow].componentCode || "").trim() ===
                record.componentCode);
  ...
  const industryStats = React.useMemo(() => {
    ...
    tableData.forEach((product) => {
      ...
      if (product.productComplianceStatus === "Compliant")
        stats[category].compliantSKUs++;
      else if (product.productComplianceStatus === "Non-Compliant")
        stats[category].nonCompliantSKUs++;

      // Component Stats
      if (product.details) {
        product.details.forEach((comp) => {
          stats[category].totalComponents++;
          if (comp.componentComplianceStatus === "Compliant")
            stats[category].compliantComponents++;
          else if (comp.componentComplianceStatus === "Non-Compliant")
            stats[category].nonCompliantComponents++;
        });
      }
    });
  ...
  const analysisColumns = [
    ...
    {
      title: "Compliant SKUs",
      dataIndex: "compliantSKUs",
      key: "compliantSKUs",
      align: "right",
      render: (val) => (
    ...
    {
      title: "Non-Compliant SKUs",
      dataIndex: "nonCompliantSKUs",
      key: "nonCompliantSKUs",
      align: "right",
    ...
    },
    {
      ...
      dataIndex: "compliantComponents",
      key: "compliantComponents",
      align: "right",
      render: (val) => (
        <span className="text-green-600 font-medium">{val}</span>
      ),
    ...
    {
      title: "Non-Compliant Components",
      dataIndex: "nonCompliantComponents",
      key: "nonCompliantComponents",
      align: "right",
      render: (val) => <span className="text-red-600 font-medium">{val}</span>,
    },
  ];
            ...
            summary={(pageData) => {
              ...
              return (
                <Table.Summary.Row className="bg-gray-50 font-bold">
                  ...
                  <Table.Summary.Cell align="right" className="text-green-700">
                    {compliantSKUs}
                  </Table.Summary.Cell>
                  <Table.Summary.Cell align="right" className="text-red-700">
                    {nonCompliantSKUs}
                  </Table.Summary.Cell>
                  <Table.Summary.Cell align="right">
                    {totalComponents}
    ...
    return (
      <>
        <style>{`
                ...
                `}</style>
        <div className="summary-report-container">{summaryTableContent}</div>
      </>
    );
  }
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Review overall compliance and procurement summary
            </p>
          </div>
