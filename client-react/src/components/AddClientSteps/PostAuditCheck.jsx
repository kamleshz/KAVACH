...
const HoverRow = ({ children, ...props }) => {
            ...
            className={`font-semibold ${
              record.complianceStatus === "Compliant"
                ...
                : record.complianceStatus === "Non-Compliant"
                  ? "text-red-600"
                  : record.complianceStatus === "Partially Compliant"
                    ? "text-amber-600"
                    : "text-gray-600"
            }`}
          >
        ...
        <div className="col-span-2">
          ...
          {hasRemarksArray ? (
            ...
            <div className="mt-1 space-y-1">
              ...
              {auditorRemark && (
                <div>
                  <span className="text-gray-400 text-[10px]">Auditor:</span>{" "}
                  {auditorRemark}
                </div>
              )}
            </div>
  ...
  return (
    ...
    >
      <tr
        {...restProps}
        className={`${restProps.className} hover:bg-blue-50 transition-colors duration-200 cursor-pointer`}
      >
        {children}
      </tr>
    </Popover>
...
const PostAuditCheck = ({
  ...
  const [plasticAnalysisTab, setPlasticAnalysisTab] =
    useState("prePostValidation");
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      await queueReportAndDownload({
        ...
        filename: `Plastic_Compliance_Report_${clientId}.pdf`,
      });
  ...
  const handleSaveProductAssessment = async () => {
    try {
      ...
      await api.post(
        API_ENDPOINTS.CLIENT.PRODUCT_COMPLIANCE_ROW_SAVE(clientId),
        {
          type: applicationType,
          itemId: selectedPlantId,
    ...
    const fetchSummaryData = async () => {
      if (postValidationActiveTab === "productAssessment" && clientId) {
        try {
          ...
          const [prodRes, compRes, suppRes, monthlyRes, recycledRes] =
            await Promise.all([
  ...
  const [localUrepSelectedYear, setLocalUrepSelectedYear] =
    useState(urepSelectedYear);
  const [localCostAnalysisSubTab, setLocalCostAnalysisSubTab] =
    useState(costAnalysisSubTab);
  ...
  const finalUrepYearOptions = urepYearOptions || [
  ...
  const normalizedMonthlyProcurementRaw = React.useMemo(() => {
    const rows = Array.isArray(monthlyProcurementRaw)
      ? monthlyProcurementRaw
      : [];
    return rows.map((row) => {
  ...
  const procurementAttrsBySystemCode = React.useMemo(() => {
    ...
    for (const r of rows) {
      ...
      const foodGrade = pickText(
        r?.foodGrade,
        r?.["Food Grade"],
        r?.["FOOD GRADE"],
      );
      const recycledPolymerUsed = pickText(
        r?.recycledPolymerUsed,
        r?.["Recycled Polymer Used"],
        r?.["RECYCLED POLYMER USED"],
      );
      const polymerType = pickText(
        r?.polymerType,
        r?.["Polymer Type"],
        r?.["POLYMER TYPE"],
      );
      ...
      if (!next.recycledPolymerUsed && recycledPolymerUsed)
        next.recycledPolymerUsed = recycledPolymerUsed;
      if (!next.polymerType && polymerType) next.polymerType = polymerType;
      if (!next.componentPolymer && componentPolymer)
        next.componentPolymer = componentPolymer;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
      ...
      {clientId ? (
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Review post audit observations and close marking and labelling
                points.
              </p>
            </div>
          </div>
          <div className="mb-6">
              ...
              <Select
                ...
                options={[
                  ...
                  ...(wasteType === "Plastic" || wasteType === "Plastic Waste"
                    ? [
                        {
                          value: "plasticSpecific",
                          label: "Plastic Specific Analysis",
                        },
                      ]
                    : []),
                ]}
            ...
            <div className="hidden md:block w-full rounded-2xl border border-gray-200 bg-gray-100 p-1">
              <div
                className={`grid ${wasteType === "Plastic" ? "grid-cols-8" : "grid-cols-7"} gap-1`}
              >
                <button
                  type="button"
                  onClick={() =>
                    setPostValidationActiveTab("productAssessment")
                  }
                ...
                {(wasteType === "Plastic" || wasteType === "Plastic Waste") && (
                  <button
                    type="button"
                    onClick={() =>
                      setPostValidationActiveTab("plasticSpecific")
                    }
                    className={`w-full rounded-xl px-4 py-2 text-xs font-semibold leading-tight transition-all ${
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                  <div className="text-left">
                    ...
                    <p className="text-gray-500 text-xs">
                      Review product assessment details.
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    >
                      {savingSummary ? <LoadingOutlined spin /> : <FaSave />}{" "}
                      Save Changes
                    </button>
                  </div>
                </div>
              ...
              <div className="border border-gray-200 rounded-xl bg-white p-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                  <div className="text-left">
                    ...
                    <p className="text-gray-500 text-xs">
                      Data is fetched from Product Compliance and Component
                      Details.
                    </p>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mt-4 gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    {[
                      postValidationPagination.current - 1,
                      postValidationPagination.current,
                      postValidationPagination.current + 1,
                    ]
                  ...
                  <div className="flex items-center gap-3 text-xs sm:text-sm text-gray-600">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setPostValidationGotoPage(e.target.value)
                        }
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-xs">
                          Track and manage SKU compliance status
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
                    ...
                    <div className="bg-white rounded-lg border border-amber-200 p-3 shadow-sm">
                      ...
                      <p className="text-xl font-bold text-amber-600 mt-1">
                        {skuComplianceData?.filter(
                          (r) => r.complianceStatus === "Partially Compliant",
                        ).length || 0}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-4">
                  <Table
                    ...
                    dataSource={(() => {
                      ...
                      let dataToFilter =
                        skuTableDataSource && skuTableDataSource.length > 0
                          ? skuTableDataSource
                          : skuComplianceData || [];
                      ...
                      if (skuStatusFilter !== "all") {
                        if (skuStatusFilter === "notset") {
                          dataToFilter = dataToFilter.filter(
                            (r) => !r.complianceStatus,
                          );
                        } else {
                          dataToFilter = dataToFilter.filter(
                            (r) => r.complianceStatus === skuStatusFilter,
                          );
                        }
                      }
                      return dataToFilter;
                    ...
                    pagination={{
                      ...
                      onChange: (page, pageSize) =>
                        setSkuPagination({ current: page, pageSize }),
                      className: "mt-4",
                    }}
                    scroll={{ x: 2900 }}
                    size="small"
                    ...
                    rowKey={(record) =>
                      record.key || record._id || Math.random()
                    }
                    locale={{
                      emptyText: (
                ...
                <div className="mb-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Month-wise purchase (MT) and recycled quantity.
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="inline-flex items-center rounded-full bg-gray-100 p-1">
                        <button
                          type="button"
                          onClick={() =>
                            setMonthlyProcurementDisplayMode("graph")
                          }
                          className={`
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setMonthlyProcurementDisplayMode("table")
                          }
                          className={`
                                                            inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold transition-all
                                                            ${
                    ...
                    <div className="border border-orange-200 bg-orange-50 rounded-lg p-3">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="relative">
                          <button
                            type="button"
                            onClick={() =>
                              setMonthlyProcurementFilterOpen((prev) => ({
                                ...prev,
                          ...
                          >
                            ...
                            <FaChevronDown
                              className={`text-orange-400 transition-transform duration-200 ${monthlyProcurementFilterOpen.category ? "rotate-180" : ""}`}
                            />
                          </button>
                            ...
                            <div className="absolute z-20 mt-2 w-56 rounded-md border border-orange-200 bg-white shadow-lg p-2">
                              ...
                              <label className="flex items-center gap-2 py-1 text-xs">
                                <input
                                  type="checkbox"
                                  checked={
                                    (monthlyProcurementFilters.category || [])
                                      .length === 0
                                  }
                                  onChange={() =>
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <FaChevronDown
                              className={`text-orange-400 transition-transform duration-200 ${monthlyProcurementFilterOpen.polymer ? "rotate-180" : ""}`}
                            />
                          </button>
                            ...
                            <div className="absolute z-20 mt-2 w-56 rounded-md border border-orange-200 bg-white shadow-lg p-2">
                              ...
                              <label className="flex items-center gap-2 py-1 text-xs">
                                <input
                                  type="checkbox"
                                  checked={
                                    (monthlyProcurementFilters.polymer || [])
                                      .length === 0
                                  }
                                  onChange={() =>
                                    setMonthlyProcurementFilters((prev) => ({
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <FaChevronDown
                              className={`text-orange-400 transition-transform duration-200 ${monthlyProcurementFilterOpen.quarter ? "rotate-180" : ""}`}
                            />
                          </button>
                            ...
                            <div className="absolute z-20 mt-2 w-56 rounded-md border border-orange-200 bg-white shadow-lg p-2">
                              ...
                              <label className="flex items-center gap-2 py-1 text-xs">
                                <input
                                  type="checkbox"
                                  checked={
                                    (monthlyProcurementFilters.quarter || [])
                                      .length === 0
                                  }
                                  onChange={() =>
                                    setMonthlyProcurementFilters((prev) => ({
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <FaChevronDown
                              className={`text-orange-400 transition-transform duration-200 ${monthlyProcurementFilterOpen.half ? "rotate-180" : ""}`}
                            />
                          </button>
                            ...
                            <div className="absolute z-20 mt-2 w-56 rounded-md border border-orange-200 bg-white shadow-lg p-2">
                              ...
                              <label className="flex items-center gap-2 py-1 text-xs">
                                <input
                                  type="checkbox"
                                  checked={
                                    (monthlyProcurementFilters.half || [])
                                      .length === 0
                                  }
                                  onChange={() =>
                                    setMonthlyProcurementFilters((prev) => ({
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        <div className="flex flex-wrap gap-2">
                          {(monthlyProcurementFilters.category || []).length >
                            0 && (
                            <span className="inline-flex items-center px-2 py-1 rounded-full bg-orange-50 border border-orange-200 text-[11px] text-orange-700">
                              Industry Category:{" "}
                              {(monthlyProcurementFilters.category || []).join(
                                ", ",
                              )}
                            </span>
                          )}
                          {(monthlyProcurementFilters.polymer || []).length >
                            0 && (
                            <span className="inline-flex items-center px-2 py-1 rounded-full bg-orange-50 border border-orange-200 text-[11px] text-orange-700">
                              Polymer:{" "}
                              {(monthlyProcurementFilters.polymer || []).join(
                                ", ",
                              )}
                            </span>
                          )}
                          {(monthlyProcurementFilters.quarter || []).length >
                            0 && (
                            <span className="inline-flex items-center px-2 py-1 rounded-full bg-orange-50 border border-orange-200 text-[11px] text-orange-700">
                              Quarter:{" "}
                              {(monthlyProcurementFilters.quarter || []).join(
                                ", ",
                              )}
                            </span>
                          )}
                          {(monthlyProcurementFilters.half || []).length >
                            0 && (
                            <span className="inline-flex items-center px-2 py-1 rounded-full bg-orange-50 border border-orange-200 text-[11px] text-orange-700">
                              Half:{" "}
                              {(monthlyProcurementFilters.half || []).join(
                                ", ",
                              )}
                            </span>
                          )}
                  ...
                  {monthlyProcurementLoading && (
                    <div className="text-xs text-gray-500">
                      Loading monthly procurement data...
                    </div>
                  )}
                  ...
                  {!monthlyProcurementLoading && !monthlyProcurementError && (
                    <>
                        ...
                        <div className="overflow-x-auto mb-8">
                          <div className="min-w-[480px] max-w-4xl mx-auto px-4 pt-6">
                            {(() => {
                                  ...
                                  <table className="min-w-full divide-y divide-gray-200 text-xs">
                                    ...
                                    <tbody className="divide-y divide-gray-100 bg-white">
                                      {monthlyProcurementSummary.map((item) => {
                                        const purchase =
                                          Number(item.monthlyPurchaseMt) || 0;
                                        const recycled =
                                          Number(item.recycledQty) || 0;
                                        return (
                                          <tr key={item.month}>
                              ...
                              const maxValue = monthlyProcurementSummary.reduce(
                                (max, item) => {
                                  const purchase =
                                    Number(item.monthlyPurchaseMt) || 0;
                                  const recycled =
                                ...
                                <div className="relative h-96 w-full bg-white rounded-lg p-4 border border-gray-100 shadow-sm mt-2">
                                  ...
                                  <div className="absolute inset-0 left-12 right-4 top-28 bottom-8 flex flex-col justify-between pointer-events-none">
                                    {[...gridValues].reverse().map((val, i) => (
                                      <div
                                        key={i}
                                        className="relative w-full h-px bg-gray-100 border-t border-dashed border-gray-200"
                                      >
                                    ...
                                    {monthlyProcurementSummary.map((item) => {
                                      ...
                                      const totalHeight =
                                        total && maxValue
                                          ? (total / maxValue) * 100
                                          : 0;
                                      const purchaseHeight = total
                                        ? (purchase / total) * 100
                                        : 0;
                                      const recycledHeight = total
                                        ? (recycled / total) * 100
                                        : 0;
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-gray-900/95 backdrop-blur-sm text-white text-[10px] rounded-lg py-2 px-3 opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-20 shadow-xl translate-y-2 group-hover:translate-y-0">
                                            <div className="font-bold text-xs border-b border-gray-700 pb-1 mb-1 text-gray-100">
                                              {item.month}
                                            </div>
                      ...
                      {(() => {
                        const polymerSummaryResult =
                          buildPolymerProcurementSummary(
                            normalizedMonthlyProcurementRaw,
                            monthlyProcurementFilters,
                            monthlyProcurementViewMode,
                          );
                        const categorySummaryResult =
                          buildCategoryProcurementSummary(
                            normalizedMonthlyProcurementRaw,
                            monthlyProcurementFilters,
                            monthlyProcurementViewMode,
                        ...
                        const polymerData = (
                          polymerSummaryResult.data || []
                        ).map((row) => ({
                          ...row,
                          monthlyPurchaseMt:
                            row.monthlyPurchaseMt === "-"
                              ? 0
                              : Number(row.monthlyPurchaseMt) || 0,
                          recycledQty:
                            row.recycledQty === "-"
                              ? 0
                              : Number(row.recycledQty) || 0,
                        }));
                        const polymerKeys = polymerSummaryResult.keys || [];

                        const categoryData = (
                          categorySummaryResult.data || []
                        ).map((row) => ({
                          ...row,
                          monthlyPurchaseMt:
                            row.monthlyPurchaseMt === "-"
                        ...
                        if (!polymerData.length && !categoryData.length) {
                          return (
                            <div className="text-xs text-gray-500">
                              No polymer/category-wise procurement data
                              available.
                            </div>
                          );
                        }
                                  ...
                                  <table className="min-w-full divide-y divide-gray-200 text-xs">
                                    ...
                                    <tbody className="divide-y divide-gray-100 bg-white">
                                      {polymerData.length > 0 ? (
                                        polymerData.map((row, i) => {
                                          const total =
                                            (row.monthlyPurchaseMt || 0) +
                                            (row.recycledQty || 0);
                                          return (
                                            <tr key={i}>
                                              <td className="px-3 py-1.5 text-gray-800 font-medium sticky left-0 bg-white z-10 shadow-sm">
                                                {row.label}
                                              </td>
              ...
              <div className="border border-gray-200 rounded-xl bg-white p-6 space-y-6">
                <div>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
                    <div>
                      <p className="text-gray-800 font-semibold">
                        UREP Target (Normalized)
                      </p>
                      <p className="text-xs text-gray-500">
                        Category-wise UREP targets stored as Category + Year +
                        Target Value.
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          handleLocalUrepYearChange(e.target.value)
                        }
                        disabled={readOnly}
                  ...
                  <div className="overflow-x-auto">
                    {(() => {
                      const rowsForYear = finalUrepTargets.filter(
                        (row) => row.year === finalUrepSelectedYear,
                      );
                      if (!rowsForYear.length) {
                        return (
                          <div className="text-xs text-gray-500">
                            No UREP targets configured for{" "}
                            {finalUrepSelectedYear}.
                          </div>
                        );
                      }
                      return (
                ...
                <div>
                  <p className="text-gray-800 font-semibold mb-2">
                    UREP Performance Summary
                  </p>
                  <p className="text-xs text-gray-500 mb-3">
                    Comparison of UREP target percentage with actual recycled
                    performance by industry category.
                  </p>
                  <div className="overflow-x-auto">
                    {(() => {
                      const rowsForYear = urepTargets.filter(
                        (row) => row.year === urepSelectedYear,
                      );
                      if (!rowsForYear.length) {
                        return (
                          <div className="text-xs text-gray-500">
                      ...
                      if (!categorySummaryData.length) {
                        return (
                          <div className="text-xs text-gray-500">
                            No monthly procurement data available for
                      ...
                      return (
                        <table
                          ...
                          aria-label="UREP performance summary table"
                        >
                          <caption className="sr-only">
                            UREP performance summary comparing target percentage
                            with actual recycled performance by industry
                            category.
                          </caption>
                          <thead className="bg-gray-50">
                            <tr>
                              <th
                                ...
                                className="w-[200px] px-4 py-2 text-left font-semibold text-gray-700"
                              >
                                Industry Category
                              </th>
                              <th
                                ...
                                className="w-[110px] px-4 py-2 text-right font-semibold text-gray-700"
                              >
                                Target %
                              </th>
                              <th
                                scope="col"
                                aria-label="Monthly Purchase in metric tons"
                                className="w-[170px] px-4 py-2 text-right font-semibold text-gray-700"
                              >
                                Monthly Purchase (MT)
                              </th>
                              <th
                                ...
                                className="w-[130px] px-4 py-2 text-right font-semibold text-gray-700"
                              >
                                Target Qty
                              </th>
                              <th
                                scope="col"
                                aria-label="Recycled Percentage"
                                className="w-[110px] px-4 py-2 text-right font-semibold text-gray-700"
                              >
                                Recycled %
                              </th>
                              <th
                                scope="col"
                                aria-label="Recycled Quantity in metric tons"
                                className="w-[150px] px-4 py-2 text-right font-semibold text-gray-700"
                              >
                                Recycled Qty (MT)
                              </th>
                              <th
                                scope="col"
                                aria-label="Deviation between target and recycled percentage"
                                className="w-[130px] px-4 py-2 text-right font-semibold text-gray-700"
                              >
                                Deviation
                              </th>
                          ...
                          <tbody className="divide-y divide-gray-100 bg-white">
                            {rowsForYear.map((row) => {
                              ...
                              const summary =
                                summaryByCategory.get(row.category) || {};
                              const purchase =
                                Number(summary.monthlyPurchaseMt) || 0;
                              const targetQty =
                                purchase * (targetPercent / 100);
                              const recycledQty =
                                Number(summary.recycledQty) || 0;
                              const recycledPercent =
                                purchase > 0
                                  ? (recycledQty / purchase) * 100
                                  : 0;
                              const deviation = targetPercent - recycledPercent;
                              return (
                                <tr
                                  key={`summary-${row.category}-${row.year}`}
                                  className="transition-colors hover:bg-orange-50"
                                >
                                  <td className="px-4 py-1.5 text-gray-800 whitespace-nowrap">
                                    {row.category}
                                  </td>
                                  <td className="px-4 py-1.5 text-right tabular-nums text-gray-800">
                                  ...
                                  <td
                                    className={`px-4 py-1.5 text-right font-semibold tabular-nums ${deviation > 0 ? "text-red-600" : "text-green-600"}`}
                                  >
              ...
              <div className="border border-gray-200 rounded-xl bg-white flex overflow-hidden min-h-[500px]">
                ...
                <div className="w-48 bg-gray-50 border-r border-gray-200 flex flex-col">
                  {[
                    ...
                    .filter((tab) => !isProducer || tab.id !== "Product")
                    .map((tab) => (
                      <button
                        key={tab.id}
                ...
                <div className="flex-1 p-6 bg-white overflow-y-auto">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-500 text-sm mt-1 ml-11">
                      Breakdown of procurement costs and rates by{" "}
                      {costAnalysisSubTab.toLowerCase()}.
                    </p>
                  </div>
                  ...
                  {isProducer && costAnalysisSubTab === "Polymer" && (
                    <div className="mb-4 flex space-x-4 border-b border-gray-200">
                      ...
                      <button
                        onClick={() =>
                          setProducerPolymerTab("Recycled Polymer Used")
                        }
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                            {isProducer &&
                            costAnalysisSubTab === "Polymer" &&
                            producerPolymerTab === "Recycled Polymer Used"
                              ? "Recycled Polymer Used"
                      ...
                      <tbody className="bg-white divide-y divide-gray-100">
                        {(() => {
                          ...
                          const costAnalysisRows =
                            isProducer && costAnalysisSubTab === "Polymer"
                              ? Array.isArray(monthlyProcurementRaw)
                                ? monthlyProcurementRaw
                                : []
                              : Array.isArray(postValidationData)
                                ? postValidationData
                                : [];

                          const groupedData = costAnalysisRows.reduce(
                            (acc, row) => {
                              let key = "Unknown";
                              ...
                              if (costAnalysisSubTab === "Product") {
                                ...
                                const lookup = sc
                                  ? procurementAttrsBySystemCode.get(sc)
                                  : undefined;
                              ...
                              if (
                                ...
                                if (!acc[key].foodGrade && foodGrade)
                                  acc[key].foodGrade = foodGrade;
                                if (
                                  !acc[key].recycledPolymerUsed &&
                                  recycledPolymerUsed
                                )
                                  acc[key].recycledPolymerUsed =
                                    recycledPolymerUsed;
                              ...
                              if (parseFloat(row.virginRate))
                                acc[key].vRates.push(
                                  parseFloat(row.virginRate),
                                );
                              if (parseFloat(row.recycledRate))
                                acc[key].rRates.push(
                                  parseFloat(row.recycledRate),
                                );
                              if (row.recycledPercent)
                                acc[key].recycledPercents.push(
                                  parseFloat(row.recycledPercent),
                                );

                              const vCost =
                                parseFloat(row.virginQtyAmount) || 0;
                              const rCost =
                                parseFloat(row.recycledQrtAmount) || 0;
                          ...
                          return rows.map(([key, stats]) => {
                            const isSupplierTab =
                              costAnalysisSubTab === "Supplier";
                            const isPolymerTab =
                              costAnalysisSubTab === "Polymer";
                            const isCategoryTab =
                              costAnalysisSubTab === "Category";
                            const isExpandable =
                              isSupplierTab || isPolymerTab || isCategoryTab;
                            ...
                            return (
                              <React.Fragment key={key}>
                                <tr
                                  ...
                                  onClick={
                                    isExpandable ? toggleExpansion : undefined
                                  }
                                >
                                  {costAnalysisSubTab === "Component" &&
                                    !isProducer && (
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">
                                    {stats.recycledAmount?.toLocaleString(
                                      "en-IN",
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-600">
                                    {stats.virginAmount?.toLocaleString(
                                      "en-IN",
                                      { style: "currency", currency: "INR" },
                                    ) || "₹0.00"}
                                  </td>
              ...
              <div className="border border-gray-200 rounded-xl bg-white p-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
                  <div className="text-left">
                    ...
                    <p className="text-gray-500 text-xs">
                      Plastic specific analysis details.
                    </p>
                  </div>
              ...
              <div className="border border-gray-200 rounded-xl bg-white p-8">
                <div className="flex flex-col items-center justify-center text-center max-w-2xl mx-auto">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 mb-2">
                    Plastic Compliance Audit Report
                  </h2>
                  <p className="text-gray-500 mb-8">
                    Generate and download the comprehensive audit report
                    including company details, pre/post validation summary, EPR
                    targets, and detailed compliance analysis.
                  </p>

                  <div className="bg-gray-50 rounded-xl p-6 w-full border border-gray-100 mb-8">
                    <h4 className="font-semibold text-gray-700 mb-4 text-left">
                      Report Contents:
                    </h4>
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-3 text-left">
                      <li className="flex items-center gap-2 text-sm text-gray-600">
                        <FaCheckCircle className="text-green-500" /> Company &
                        Audit Details
                      </li>
                      <li className="flex items-center gap-2 text-sm text-gray-600">
                        <FaCheckCircle className="text-green-500" /> Pre/Post
                        Validation Summary
                      </li>
                      <li className="flex items-center gap-2 text-sm text-gray-600">
                        <FaCheckCircle className="text-green-500" /> EPR Target
                        Calculation
                      </li>
                      <li className="flex items-center gap-2 text-sm text-gray-600">
                        <FaCheckCircle className="text-green-500" /> Industry
                        Category Wise Details
                      </li>
                      <li className="flex items-center gap-2 text-sm text-gray-600">
                        <FaCheckCircle className="text-green-500" /> Sales &
                  ...
                  <p className="text-xs text-gray-400 mt-4">
                    This report is strictly confidential and generated based on
                    the current audit data.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900">
            Post-validation Check Unavailable
          </h3>
          <p className="text-gray-500 mt-2">
            Please save the client data and complete audit to access
      ...
      <Modal
        title={
          currentRemarkField === "complianceRemarks"
            ? "Compliance Remarks"
            : "Auditor Remarks"
        }
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
            />
            <Button type="primary" onClick={addRemark} icon={<PlusOutlined />}>
              Add
            </Button>
          </div>
            ...
            {tempRemarks.length === 0 ? (
              ...
              <ul className="list-disc pl-4 m-0">
                {tempRemarks.map((remark, index) => (
                  <li
                    key={index}
                    className="text-sm text-gray-700 mb-1 flex justify-between items-start group"
                  >
                    <span>{remark}</span>
                    <Button
                      type="text"
