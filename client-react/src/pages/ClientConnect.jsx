...
import { useClientConnectClients } from "../hooks/queries/useClientConnectClients";
...
const getWasteConfig = (type) =>
  WASTE_CONFIG[type] || WASTE_CONFIG[WASTE_TYPES.PLASTIC];
  ...
  const filteredClients = useMemo(() => {
    ...
    const bySearch = term
      ? base.filter((client) => {
          ...
          return (
            name.includes(term) || group.includes(term) || entity.includes(term)
          );
        })
      : base;
        ...
        <div className="bg-white border border-red-200 rounded-xl p-5 md:p-6 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-600">
            {error?.response?.data?.message ||
              error?.message ||
              "Client Connect data could not be loaded."}
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Select a client to view complete pre-audit details in read-only
                mode.
              </p>
            </div>
          </div>
              ...
              description={
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    Try adjusting your search or waste type filter
                  </p>
                </div>
              }
            />
                ...
                <button
                  ...
                  onMouseEnter={() => {
                    queryClient.prefetchQuery({
                      queryKey: getClientDetailQueryKey(client._id),
                      queryFn: async () => {
                        const response = await api.get(
                          API_ENDPOINTS.CLIENT.GET_BY_ID(client._id),
                          {
                            params: { _: Date.now() },
                          },
                        );
                  ...
                  onClick={() =>
                    navigate(`/dashboard/client-connect/${client._id}`, {
