...
Date: ___________________`;
  ...
  const isTabComplete = (num) => {
    ...
    if (num === 4) {
      const docs = (client.documents || []).filter(d => ['PAN', 'GST', 'CIN', 'Factory License', 'EPR Certificate', 'Engagement Letter'].includes(d.documentType));
  ...
  const generateValidationPDF = (clientData, validationDetails) => {
    ...
    // FILTER OUT Engagement Letter
    const docRows = (clientData.documents || [])
        .filter(d => d.documentType !== 'Engagement Letter')
  ...
  const handleCompleteValidation = async () => {
    ...
    try {
      ...
      // 4. Documents
      const docs = (client.documents || []).filter(d => ['PAN', 'GST', 'CIN', 'Factory License', 'EPR Certificate'].includes(d.documentType));
      const companyDocuments = docs.length === 0 ? true : docs.every((_, i) => isVerified(`doc_${i}`));
            ...
            <div className="w-full mx-auto">
              <div className="rounded-xl border border-gray-200 bg-white">
                ...
                <div className="p-6 space-y-3">
                  ...
                  {(client.documents || []).filter(d => ['PAN', 'GST', 'CIN', 'Factory License', 'EPR Certificate'].includes(d.documentType)).length === 0 && (
