...
const AssignModal = ({ isOpen, onClose, onConfirm, clientName, users, isAssigning }) => {
      ...
      <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                ...
                <div className="mt-4 space-y-4">
                  <p className="text-sm text-gray-500 mb-2">
                    Select a user to assign <span className="font-bold text-gray-800">{clientName}</span> to.
                  </p>
                    ...
                    >
                      ...
                      {users.map((user) => (
                        <option key={user._id} value={user._id}>
                          {user.name} ({user.email}) - {user.role?.name || user.role}
                        </option>
                      ))}
                    </select>
                  </div>
          ...
          <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
              type="button"
              className={`w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm ${isAssigning || !selectedUser ? 'opacity-75 cursor-not-allowed' : ''}`}
              onClick={() => onConfirm(selectedUser, auditStartDate, auditEndDate)}
...
const ActionIcons = ({ onEdit, onView, onDelete, onAssign, showAssign }) => {
  return (
    <div className="flex items-center justify-end gap-3">
      {showAssign && (
        <button
          onClick={onAssign}
          className="group relative p-2 rounded-full text-gray-400 hover:text-green-600 hover:bg-green-50 transition-all duration-200"
        >
          <i className="fas fa-user-plus text-lg"></i>
          <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-white bg-gray-900 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">Assign User</span>
        </button>
      )}
      <button
        onClick={onEdit}
        className="group relative p-2 rounded-full text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-all duration-200"
      >
        <i className="fas fa-edit text-lg"></i>
        <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-white bg-gray-900 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">Edit Client</span>
      </button>
      <button
        onClick={onDelete}
        className="group relative p-2 rounded-full text-gray-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200"
      >
        <i className="fas fa-trash-alt text-lg"></i>
        <span className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-white bg-gray-900 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">Delete</span>
      </button>
    </div>
  );
};
...
const Clients = () => {
  ...
  const fetchUsers = async (signal) => {
    try {
      const response = await api.get(API_ENDPOINTS.USER.GET_ALL, { signal });
  ...
  const uniqueEntityTypes = useMemo(() => {
    const types = clients.map(client => client.entityType).filter(Boolean);
  ...
  const promptAssignClient = (client) => {
    setClientToAssign(client);
    setIsAssignModalOpen(true);
  };

  const confirmAssignClient = async (userId, auditStartDate, auditEndDate) => {
    if (!clientToAssign || !userId) return;

    setIsAssigning(true);
    try {
      const response = await api.patch(API_ENDPOINTS.CLIENT.ASSIGN(clientToAssign._id), { 
        assignedTo: userId,
        auditStartDate,
        auditEndDate
      });
      ...
      alert(error.response?.data?.message || 'Failed to assign client.');
    } finally {
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Clients Portfolio</h1>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <tr key={client._id} className="group hover:bg-blue-50/30 transition-colors duration-200">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap text-right text-sm font-medium">
                        <div className="transition-opacity duration-200">
                            ...
                            onAssign={() => promptAssignClient(client)}
                            showAssign={user?.role?.name === 'ADMIN' || user?.role === 'ADMIN'}
                            />
                        </div>
                      </td>
                    </tr>
            ...
            <div className="bg-gray-50/50 px-8 py-4 border-t border-gray-100 flex items-center justify-between">
                <div className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                    Showing <span className="text-gray-900">{filteredClients.length}</span> of <span className="text-gray-900">{clients.length}</span> clients
                </div>
      ...
      <AssignModal
        ...
        onConfirm={confirmAssignClient}
        clientName={clientToAssign?.clientName}
        users={users}
