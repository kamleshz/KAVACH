...
import ProcessTable from "../features/plantProcess/components/ProcessTable";
import SupplierCompliance from "../features/plantProcess/components/SupplierCompliance";
import ProcurementPanel from "../features/plantProcess/components/ProcurementPanel";
import PlasticAnalysisPanel from "../features/plantProcess/components/PlasticAnalysisPanel";
import { resolveClientFileUrl } from "../utils/fileAccess";
import {
  usePlantProcessClientQuery,
  usePlantProcessHistoryQuery,
...
const PlantProcess = ({
  ...
  const hasValidProcessSelection =
    Boolean(clientId) &&
    (type === "CTE" || type === "CTO") &&
    /^[0-9a-fA-F]{24}$/.test((itemId || "").toString().trim());
  const navigate = useNavigate();
  const { user, isManager } = useAuth();
  const resolvedUserName = useMemo(
    () => user?.name || user?.username || user?.email || "Current User",
    [user],
  );
  ...
  const appendPersistedHistory = (entries) => {
    if (!historyStorageKey || !Array.isArray(entries) || entries.length === 0)
      return;
  ...
  const historyQuery = usePlantProcessHistoryQuery({ clientId, type, itemId });
  const clientQuery = usePlantProcessClientQuery(clientId);

  useEffect(() => {
    const rows = historyQuery.data || [];
    setDbHistory(Array.isArray(rows) ? rows : []);
    setDbHistoryLoaded(historyQuery.isFetched || historyQuery.isError);
  }, [historyQuery.data, historyQuery.isFetched, historyQuery.isError]);
  ...
  const normalizedDbHistory = useMemo(() => {
    ...
    return dbHistory.map((entry, idx) => {
      ...
      return {
        ...
        id:
          entry.id ||
          `db-${entry.table || "unknown"}-${entry.row || idx}-${entry.field || "unknown"}-${idx}`,
      };
    });
  }, [dbHistory]);
  ...
  useEffect(() => {
    ...
    api
      .post(API_ENDPOINTS.CLIENT.PRODUCT_COMPLIANCE_HISTORY_IMPORT(clientId), {
        type,
        itemId,
  ...
  const notify = (type, text, duration = 4000) => {
    ...
    setTimeout(() => {
      setNotifications((prev) =>
        prev.map((n) => (n.id === id ? { ...n, started: true } : n)),
      );
    }, 20);
    if (duration > 0) {
      setTimeout(() => {
  ...
  const ensureSupplierCodeWithState = (rows, row, excludeIndex) => {
    if (!row || row.generateSupplierCode !== "No")
      return (row?.supplierCode || "").trim();
    const supplierName = (row.supplierName || "").trim();
    if (!supplierName) return (row.supplierCode || "").trim();

    const prefix = getSupplierCodePrefix(
      row.supplierName,
      row.supplierState,
      client?.clientName,
    );
  ...
  const componentOptions = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([code, description]) => ({
      code,
      description,
    }));
  }, [productRows]);
  ...
  const systemCodeOptions = useMemo(() => {
    ...
    productRows.forEach((r) => {
      ...
      if (!existing.componentCode && r.componentCode)
        existing.componentCode = r.componentCode;
      if (!existing.componentDescription && r.componentDescription)
        existing.componentDescription = r.componentDescription;
      if (!existing.supplierName && r.supplierName)
        existing.supplierName = r.supplierName;
      if (!existing.supplierState && r.supplierState)
        existing.supplierState = r.supplierState;
      if (!existing.supplierType && r.supplierType)
        existing.supplierType = r.supplierType;
      if (!existing.supplierCategory && r.supplierCategory)
        existing.supplierCategory = r.supplierCategory;
      ...
      if (code) {
        ...
        if (!existing || (hasImage && !existingHasImage)) {
          map.set(code, {
            skuCode: code,
            skuDescription:
              row.skuDescription || existing?.skuDescription || "",
  ...
  const handleRowChange = (index, field, value) => {
    setProductRows((prev) => {
      ...
      if (
        updatedRow.generate === "No" &&
        (field === "skuCode" || field === "componentDescription")
      ) {
        const sku = (updatedRow.skuCode || "").trim();
        const desc = (updatedRow.componentDescription || "").trim();

        if (sku && desc) {
          const existingMatch = copy.find(
            (r, i) =>
              i !== index &&
              (r.skuCode || "").trim() === sku &&
          ...
          // Because C1 is for Bottle.
            ...
            copy.forEach((r) => {
              // Check ALL rows including current (current might have old code we are replacing, but that's fine, we want max)
      ...
      if (
        updatedRow.generateSupplierCode === "No" &&
        (field === "supplierName" || field === "supplierState")
      ) {
  ...
  const handleProductComponentCodeChange = (index, code) => {
    const compMatch = componentRows.find(
      (r) =>
        (r.componentCode || "").trim().toLowerCase() ===
    ...
    setProductRows((prev) => {
      ...
      copy[index] = {
        ...
        componentCode: code,
        componentDescription:
          descFromComponent || copy[index].componentDescription,
  ...
  const generateComponentCodeForBulk = (
    currentRows,
    clientName,
    plantName,
  ...
  const generateSupplierCodeForBulk = (
    currentRows,
    supplierName,
    clientName,
  ) => {
    const supplierShortName = getSupplierShortName(supplierName);
    const companyShortName = getCompanyShortName(clientName);
    const sampleRow = (currentRows || []).find(
      (r) =>
        (r.supplierName || "").trim().toLowerCase() ===
  ...
  const handleProductDeleteAll = async () => {
    ...
    try {
      ...
      notify(
        "error",
        err.response?.data?.message || "Failed to delete all rows",
      );
    } finally {
  ...
  const handleSupplierDeleteAll = async () => {
    ...
    try {
      ...
      const res = await api.post(
        API_ENDPOINTS.CLIENT.PRODUCT_SUPPLIER_COMPLIANCE(clientId),
        payload,
      );
      if (res.data.success) {
  ...
  const handleComponentSummaryChange = (
    skuIdentifier,
    componentCode,
    field,
    value,
    ...
    setProductRows((prev) =>
      prev.map((row) => {
        const rowKey = isProducer
          ...
          : (row.skuCode || "").trim();
        if (
          rowKey === skuIdentifier &&
          (row.componentCode || "").trim() === componentCode
        ) {
          return { ...row, [field]: value };
        }
        return row;
  ...
  const handleComponentSummaryFileChange = (
    skuIdentifier,
    componentCode,
    file,
  ) => {
  ...
  const handleSupplierTemplateDownload = () => {
    ...
    const headerIndex = (label) =>
      headers.findIndex(
        (h) =>
          (h || "").toString().trim().toLowerCase() === label.toLowerCase(),
      );
    ...
    ws["!dataValidation"] = [
      {
        ...
        sqref:
          statusCol >= 0
            ? `${colLetter(statusCol)}2:${colLetter(statusCol)}500`
            : "A1:A1",
        ...
        : []),
      {
        ...
        sqref:
          foodCol >= 0
            ? `${colLetter(foodCol)}2:${colLetter(foodCol)}500`
            : "A1:A1",
  ...
  const handleExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      data.forEach((row, index) => {
        ...
        if (
          componentCode &&
          (componentCode.toLowerCase() === "yes" ||
            componentCode.toLowerCase() === "no")
        ) {
  ...
  const handleBulkSave = async () => {
    ...
    try {
      ...
      if (invalidRows.length > 0) {
        notify(
          "warning",
          `${invalidRows.length} row(s) have missing fields. Please fix them.`,
        );
      ...
      const payload = validRows.map((r) => {
        ...
        return {
          ...rest,
          productImage:
            typeof rest.productImage === "string" ? rest.productImage : "",
          componentImage:
  ...
  const handleSupplierExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      data.forEach((row) => {
        ...
        if (systemCode) {
          selectedOption =
            systemCodeOptions.find((opt) => opt.code === systemCode) || null;
          if (selectedOption && selectedOption.data) {
            if (!componentCode)
              componentCode = selectedOption.data.componentCode || "";
            if (!componentDescription)
              componentDescription =
                selectedOption.data.componentDescription || "";
            if (!supplierName)
              supplierName = selectedOption.data.supplierName || "";
            if (!supplierState)
              supplierState = selectedOption.data.supplierState || "";
            if (!supplierType)
              supplierType = selectedOption.data.supplierType || "";
          }
        }
  ...
  const handleSupplierBulkSave = async () => {
    ...
    try {
      ...
      if (validRows.length === 0 && invalidRows.length > 0) {
        ...
        notify(
          "warning",
          "No valid rows to save. Please check for missing fields.",
        );
        setIsSupplierBulkSaving(false);
        return false;
      }
      ...
      const payload = {
        ...
        rows: validRows.map((r) => ({
          ...
          ctoStartDate: r.ctoStartDate,
          ctoValidUpto: r.ctoValidUpto,
          ctoCcaDocument: r.ctoCcaDocument,
        })),
      };
      ...
      notify(
        "error",
        err.response?.data?.message || "Failed to save supplier rows",
      );
      return false;
    } finally {
      setIsSupplierBulkSaving(false);
  ...
  const isProductFieldChanged = (rowIndex, field, currentValue) => {
    ...
    return (
      getComparableProductValue(prevValue, field) !==
      getComparableProductValue(currentValue, field)
    );
  };
  ...
  const handleSkuCodeSelect = (idx, selectedSkuCode) => {
    const selectedOption = skuOptions.find(
      (opt) => opt.skuCode === selectedSkuCode,
    );
    ...
    newRows[idx] = {
      ...
      productImage: selectedOption
        ? selectedOption.productImage
        : newRows[idx].productImage,
    };
    setSkuRows(newRows);
  };
  ...
  const saveRow = async (idx) => {
    ...
    const isDuplicate = productRows.some((r, i) => {
      ...
      const sameSku =
        (r.skuCode || "").trim() === (rowToCheck.skuCode || "").trim();
      const sameDesc =
        (r.componentDescription || "").trim() ===
        (rowToCheck.componentDescription || "").trim();
    ...
    if (isDuplicate) {
      notify(
        "error",
        "Component Code must be unique (or match existing SKU/Description)",
      );
      setSavingRow(null);
      return;
    }
    ...
    if (skuToCheck && descToCheck) {
      ...
      if (existingMatch) {
        ...
        if (existingCode && existingCode !== codeToCheck) {
          notify(
            "error",
            `For this SKU & Description, you must reuse Component Code: ${existingCode}`,
          );
          setSavingRow(null);
          return;
        }
    ...
    const supplierCodeNormalized = ensureSupplierCodeWithState(
      productRows,
      rowToCheck,
      idx,
    );
    const supplierCodeToCheck = supplierCodeNormalized;
    const supplierNameToCheck = (rowToCheck.supplierName || "")
      .trim()
      .toLowerCase();
    ...
    if (supplierCodeToCheck) {
      ...
      if (isSupplierDuplicate) {
        notify(
          "error",
          "Supplier Code must be unique (or reused for the same Supplier)",
        );
        setSavingRow(null);
        return;
      }
    ...
    try {
      ...
      if (
        (productRows[idx]?.supplierCode || "").trim() !== supplierCodeNormalized
      ) {
        setProductRows((prev) => {
          const copy = [...prev];
          copy[idx] = row;
      ...
      const hasFiles =
        row.productImage instanceof File || row.componentImage instanceof File;
      ...
      if (hasFiles) {
        ...
        const rowJson = JSON.stringify({
          ...row,
          productImage:
            typeof row.productImage === "string" ? row.productImage : "",
          componentImage:
            typeof row.componentImage === "string" ? row.componentImage : "",
        });
        fd.append("row", rowJson);
        if (row.productImage instanceof File)
          fd.append("productImage", row.productImage);
        if (row.componentImage instanceof File)
          fd.append("componentImage", row.componentImage);

        console.log("Uploading row with files:", {
          rowIndex: idx,
          hasProductImage: row.productImage instanceof File,
        });

        const res = await api.post(
          API_ENDPOINTS.CLIENT.PRODUCT_COMPLIANCE_UPLOAD(clientId),
          fd,
          {
      ...
      fields.forEach((field) => {
        ...
        if (
          getComparableProductValue(prevVal, field) !==
          getComparableProductValue(currVal, field)
        ) {
          historyEntries.push({
      ...
      notify(
        "error",
        err.response?.data?.message || "Failed to save product row",
      );
    } finally {
      setSavingRow(null);
    }
  ...
  const saveSkuRow = async (idx) => {
    ...
    try {
      ...
      if (hasFiles) {
        ...
        const res = await api.post(
          API_ENDPOINTS.CLIENT.SKU_COMPLIANCE_UPLOAD(clientId),
          fd,
          {
            headers: { "Content-Type": "multipart/form-data" },
          },
  ...
  const handleSkuBulkSave = async () => {
    ...
    try {
      ...
      const res = await api.post(
        API_ENDPOINTS.CLIENT.SKU_COMPLIANCE(clientId),
        {
          type,
          itemId,
          rows: payload,
  ...
  const handleSkuDeleteAll = async () => {
    if (
      !window.confirm(
        "Are you sure you want to delete all SKU compliance rows?",
      )
    )
      return;
  ...
  const handleSkuExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      data.forEach((row, index) => {
        ...
        let skuUom = getValue([/sku.*uom/i, /uom/i]);
        let generate =
          getValue([/^generate(?!.*supplier)/i, /^generate$/i]) || "No";
        ...
        if (generateSupplierCode === "Yes") {
          const match = currentAllRows.find(
            (r) =>
              (r.supplierName || "").trim().toLowerCase() ===
                supplierName.toLowerCase() && (r.supplierCode || "").trim(),
          );
  ...
  const handleComponentChange = (index, field, value) => {
    if (field === "systemCode" && value) {
      const isDuplicate = componentRows.some(
        (r, i) =>
          i !== index && (r.systemCode || "").trim() === (value || "").trim(),
      );
      if (isDuplicate) {
    ...
    setComponentRows((prev) => {
      ...
      if (
        field === "polymerType" &&
        (value || "").toString().toUpperCase() !== "PET"
      ) {
        if (
      ...
      if (field === "systemCode") {
        ...
        if (selected && selected.data) {
          ...
          copy[index].componentDescription =
            selected.data.componentDescription || "";
          copy[index].supplierName = selected.data.supplierName || "";
          if (selected.data.polymerType)
            copy[index].polymerType = selected.data.polymerType;
          if (selected.data.componentPolymer)
            copy[index].componentPolymer = selected.data.componentPolymer;
          if (selected.data.category)
            copy[index].category = selected.data.category;
        }
        if (isProducer) {
          const match = (Array.isArray(supplierRows) ? supplierRows : []).find(
            (r) =>
              (r?.systemCode || "").toString().trim() ===
              (value || "").toString().trim(),
          );
  ...
  const saveComponentRow = async (idx) => {
    ...
    try {
      ...
      if (
        !isProducer &&
        pt &&
        pt.toLowerCase() !== "others" &&
        cp &&
        pt.toLowerCase() !== cp.toLowerCase()
      ) {
        notify("error", "Polymer Type and Component Polymer is not matching");
        return;
      }
      if (
        (row.recycledPolymerUsed || "").toString().trim().toLowerCase() ===
          "rpet" &&
        pt.toUpperCase() !== "PET"
      ) {
        notify(
      ...
      const resolvedRow = (() => {
        ...
        return {
          ...row,
          foodGrade: (match?.foodGrade || row?.foodGrade || "").toString(),
        };
      })();
      if (isProducer) {
        setComponentRows((prev) => {
      ...
      await api.post(
        API_ENDPOINTS.CLIENT.PRODUCT_COMPONENT_DETAILS(clientId),
        payload,
      );
      const fields = [
        "skuCode",
  ...
  const getComponentRowValidationError = (row) => {
    ...
    if (row.category === "Category I" && !row.containerCapacity)
      missing.push("Container Capacity");
  ...
  const handleComponentBulkSave = async () => {
    ...
    try {
      ...
      const errorCount = rowsToSave.reduce(
        (acc, r) => acc + (r._validationError ? 1 : 0),
        0,
      );
      ...
      if (isProducer) {
        const hasRpetMismatch = rowsToSave.some((r) => {
          ...
          const recycled = (r.recycledPolymerUsed || "")
            .toString()
            .trim()
            .toLowerCase();
      ...
      if (!isProducer) {
        ...
        if (hasMismatch) {
          ...
          notify(
            "error",
            "Polymer Type and Component Polymer must match (except Others)",
          );
          setIsComponentBulkSaving(false);
          return false;
        }
      ...
      const payload = rowsToSave.map((r) => {
        ...
        if (isProducer) {
          const match = (Array.isArray(supplierRows) ? supplierRows : []).find(
            (s) =>
              (s?.systemCode || "").toString().trim() ===
              (rest?.systemCode || "").toString().trim(),
          );
          rest.foodGrade = (
            match?.foodGrade ||
            rest?.foodGrade ||
      ...
      if (res.data && res.data.success) {
        ...
        if (errorCount > 0) {
          notify(
            "warning",
            `Saved ${savedRows.length} rows. ${errorCount} rows have issues.`,
          );
        } else {
  ...
  const handleComponentExport = () => {
    ...
    const exportData = componentRows.map((row) => {
      const data = {
        ...
        ...(isProducer
          ? { "Recycled Polymer Used": row.recycledPolymerUsed || "" }
          : {}),
  ...
  const handleComponentTemplateDownload = () => {
    const headers = [
      ...
      if (!h) return false;
      if (isProducer && (h === "SKU Code" || h === "Component Polymer"))
        return false;
      return true;
    });
    ...
    reader.onload = (evt) => {
      ...
      const newRowsRaw = data.map((row) => {
        ...
        return {
          ...
          recycledPolymerUsed: isProducer
            ? getValue([/recycled.*polymer.*used/i])
            : "",
      ...
      const newRows = newRowsRaw.map((r) => {
        ...
        if (sc) {
          ...
          if (selected && selected.data) {
            ...
            if (!rr.componentCode)
              rr.componentCode = selected.data.componentCode || "";
            if (!rr.componentDescription)
              rr.componentDescription =
                selected.data.componentDescription || "";
            if (!rr.supplierName)
              rr.supplierName = selected.data.supplierName || "";
            if (!rr.polymerType)
              rr.polymerType = selected.data.polymerType || "";
            if (!rr.componentPolymer)
              rr.componentPolymer = selected.data.componentPolymer || "";
            if (!rr.category) rr.category = selected.data.category || "";
          }
          if (isProducer) {
            const match = (
              Array.isArray(supplierRows) ? supplierRows : []
            ).find((s) => (s?.systemCode || "").toString().trim() === sc);
      ...
      setComponentRows((prev) => {
        ...
        setComponentPage(
          Math.max(1, Math.ceil(merged.length / componentItemsPerPage)),
        );
        return merged;
      });
  ...
  const [supplierCtoRows, setSupplierCtoRows] = useState([]);
  const [lastSavedSupplierCtoRows, setLastSavedSupplierCtoRows] = useState([]);
  const [savingSupplierCtoRow, setSavingSupplierCtoRow] = useState(null);

  const SUPPLIER_CTO_REGISTRATION_STATUS_OPTIONS = [
    "Approved",
    "In Progress",
    "Pending",
  ];

  const normalizeSupplierRegistrationStatus = (
    supplierStatus = "",
    registrationStatus = "",
  ) => {
    const normalizedSupplierStatus = (supplierStatus || "")
      .toString()
      .trim()
      .toLowerCase();
    const rawValue = (registrationStatus || "").toString().trim().toLowerCase();
    const matchedValue = SUPPLIER_CTO_REGISTRATION_STATUS_OPTIONS.find(
      (option) => option.toLowerCase() === rawValue,
    );
  ...
  const getSupplierStatusByName = (rows = []) => {
    const map = new Map();
    (Array.isArray(rows) ? rows : []).forEach((row) => {
      const supplierName = (row?.supplierName || "").toString().trim();
      if (!supplierName) return;
      map.set(supplierName, (row?.supplierStatus || "").toString().trim());
    });
    return map;
  };

  const createBlankSupplierCtoRow = (
    supplierName = "",
    supplierStatus = "",
  ) => ({
    supplierName: (supplierName || "").toString(),
    registrationStatus: normalizeSupplierRegistrationStatus(supplierStatus),
  ...
  const normalizeSupplierCtoRow = (row = {}, supplierStatus = "") => {
    const availabilityRaw = (row?.ctoAvailability || "Available")
      .toString()
      .trim();
    const ctoAvailability =
      availabilityRaw === "Not Available" ? "Not Available" : "Available";
    ...
    return {
      ...
      registrationStatus: normalizeSupplierRegistrationStatus(
        supplierStatus,
      ...
      ctoPlantNo:
        ctoAvailability === "Not Available"
          ? ""
          : (row?.ctoPlantNo || "").toString(),
      ctoPlantName:
        ctoAvailability === "Not Available"
          ? ""
          : (row?.ctoPlantName || "").toString(),
      ctoStartDate:
        ctoAvailability === "Not Available"
          ? null
          : normalizeDateValue(row?.ctoStartDate),
      ctoValidUpto:
        ctoAvailability === "Not Available"
          ? null
          : normalizeDateValue(row?.ctoValidUpto),
      ctoCcaDocument:
        ctoAvailability === "Not Available"
          ? ""
          : (row?.ctoCcaDocument || "").toString(),
    };
  };

  const getUniqueSupplierNames = (rows = []) =>
    Array.from(
      new Set(
        (Array.isArray(rows) ? rows : [])
          .map((r) => (r?.supplierName || "").toString().trim())
          .filter(Boolean),
      ),
    );

  const mergeSupplierCtoRows = (
    supplierNames = [],
    persistedRows = [],
    supplierStatusByName = new Map(),
  ) => {
    const normalizedNames = Array.isArray(supplierNames)
      ? supplierNames
    ...
    const normalizedRows = (Array.isArray(persistedRows) ? persistedRows : [])
      .map((row) => {
        const supplierName = (row?.supplierName || "").toString().trim();
        return normalizeSupplierCtoRow(
          row,
          supplierStatusByName.get(supplierName) || "",
        );
      })
      .filter((row) => row.supplierName || !isSupplierCtoRowEmpty(row));

    if (normalizedRows.length === 0) {
      return normalizedNames.map((name) =>
        createBlankSupplierCtoRow(name, supplierStatusByName.get(name) || ""),
      );
    }
    ...
    normalizedRows.forEach((row) => {
      const supplierName = (row?.supplierName || "").toString().trim();
      if (!supplierName) return;
      if (!rowsBySupplier.has(supplierName))
        rowsBySupplier.set(supplierName, []);
      rowsBySupplier.get(supplierName).push(row);
    });

    const merged = [];
    normalizedNames.forEach((supplierName) => {
      const existingRows = rowsBySupplier.get(supplierName);
      if (Array.isArray(existingRows) && existingRows.length > 0) {
        merged.push(...existingRows);
  ...
  const isSupplierCtoRowEmpty = (row) => {
    const r = row || {};
    const fields = [
      "ctoPlantNo",
      "ctoPlantName",
    ...
    return fields.every((f) => !(r?.[f] ?? "").toString().trim());
  };
  ...
  useEffect(() => {
    const supplierNames = getUniqueSupplierNames(supplierRows);
    const supplierStatusByName = getSupplierStatusByName(supplierRows);
    setSupplierCtoRows((prev) =>
      mergeSupplierCtoRows(supplierNames, prev, supplierStatusByName),
    );
    setLastSavedSupplierCtoRows((prev) =>
      mergeSupplierCtoRows(supplierNames, prev, supplierStatusByName),
    );
  }, [supplierRows]);
  ...
  const handleRecycledChange = (index, field, value) => {
    if (field === "systemCode" && value) {
      const isDuplicate = recycledRows.some(
        (r, i) =>
          i !== index && (r.systemCode || "").trim() === (value || "").trim(),
      );
      if (isDuplicate) {
  ...
  const handleRecycledBulkSave = async () => {
    try {
      ...
      const res = await api.post(
        API_ENDPOINTS.CLIENT.RECYCLED_QUANTITY_USED(clientId),
        payload,
      );
      const savedRows = res.data?.data || [];
  ...
  const handleRecycledDeleteAll = async () => {
    if (
      !window.confirm(
        "Are you sure you want to delete all recycled quantity rows?",
      )
    )
      return;
  ...
  const handleRecycledExcelUpload = (e) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          if (newRow.systemCode) {
            const selected = systemCodeOptions.find(
              (opt) => opt.code === newRow.systemCode,
            );
            if (selected && selected.data) {
              if (!newRow.componentCode)
                newRow.componentCode = selected.data.componentCode;
              if (!newRow.componentDescription)
                newRow.componentDescription =
                  selected.data.componentDescription;
              if (!newRow.supplierName)
                newRow.supplierName = selected.data.supplierName;
        ...
        setRecycledPage(
          Math.ceil((recycledRows.length + rows.length) / recycledItemsPerPage),
        );
    ...
    try {
      const exportData = recycledRows.map((row) => {
        const data = {
          ...
          "Used Recycled %":
            (parseFloat(row.usedRecycledPercent) * 100).toFixed(2) + "%",
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            if (dateVal === "Not Applicable") {
              ...
              if (parts.length === 3) {
                ...
                if (!isNaN(d) && !isNaN(m) && !isNaN(y) && m >= 1 && m <= 12) {
                  ...
                  newRow.monthName = dateObj.toLocaleString("default", {
                    month: "long",
                  });
          ...
          if (newRow.systemCode) {
            ...
            if (selected && selected.data) {
              if (!newRow.supplierName)
                newRow.supplierName = selected.data.supplierName || "";
              if (!newRow.supplierCategory)
                newRow.supplierCategory = selected.data.supplierCategory || "";
              if (!newRow.componentCode)
                newRow.componentCode = selected.data.componentCode || "";
            ...
            const compMatch = componentRows.find(
              (r) =>
                (r.componentCode || "").trim() ===
                (newRow.componentCode || "").trim(),
            );
            if (compMatch) {
              if (!newRow.polymerType)
                newRow.polymerType = compMatch.polymerType || "";
              if (!newRow.componentPolymer)
                newRow.componentPolymer = compMatch.componentPolymer || "";
            ...
            if (isProducer) {
              const supplierMatch = (
                ...
                (r) =>
                  (r?.systemCode || "").toString().trim() ===
                  (newRow.systemCode || "").toString().trim(),
              );
              if (!newRow.foodGrade)
                newRow.foodGrade = (
              ...
              if (!newRow.recycledPolymerUsed)
                newRow.recycledPolymerUsed = (
          ...
          newRow.monthlyPurchaseMt = monthlyMt
            ? Number(monthlyMt).toFixed(3)
            : "";
          ...
          if (!newRow.recycledQrtAmount) {
            newRow.recycledQrtAmount =
              rRate && rQtyVal ? (rQtyVal * rRate).toFixed(3) : "";
          }
          ...
          if (!newRow.virginQtyAmount) {
            newRow.virginQtyAmount =
              vRate && vQtyVal ? (vQtyVal * vRate).toFixed(3) : "";
          }
          rows.push(newRow);
        }
        setMonthlyRows((prev) => [...prev, ...rows]);
        setMonthlyPage(
          Math.ceil((monthlyRows.length + rows.length) / monthlyItemsPerPage),
        );
        notify("success", `Uploaded ${rows.length} rows successfully`);
      } catch (err) {
        console.error("Excel upload error:", err);
    ...
    try {
      ...
      const exportData = monthlyRows.map((row) => {
        const data = {
          ...
          "Recycled %":
            (parseFloat(row.recycledPercent) * 100).toFixed(2) + "%",
  ...
  const handleRecycledCodeSelect = (index, code) => {
    ...
    const compMatch = componentRows.find(
      (r) => (r.componentCode || "").trim() === (code || "").trim(),
    );
    const descFromComponent = (compMatch?.componentDescription || "").trim();
  ...
  const saveRecycledRow = async (idx) => {
    ...
    try {
      ...
      notify(
        "error",
        err.response?.data?.message || "Failed to save recycled quantity row",
      );
    } finally {
  ...
  const handleSystemCodeSelect = (index, sysCode) => {
    if (sysCode) {
      const isDuplicate = supplierRows.some(
        (r, i) =>
          i !== index && (r.systemCode || "").trim() === (sysCode || "").trim(),
      );
      if (isDuplicate) {
        notify("error", "System Code Already Used");
        return;
    ...
    const match = productRows.find(
      (r) => (r.systemCode || "").trim() === (sysCode || "").trim(),
    );
  ...
  const handleSupplierCodeSelect = (index, code) => {
    ...
    const prodMatch = productRows.find(
      (r) =>
        (r.componentCode || "").trim() === (code || "").trim() &&
        (r.supplierName || "").trim(),
    );
    const supplierFromProduct = (prodMatch?.supplierName || "").trim();
    const supplierTypeFromProduct = (prodMatch?.supplierType || "").trim();

    const existingRowsSameCode = supplierRows.filter(
      (r) => (r.componentCode || "").trim() === (code || "").trim(),
    );
    const registeredExisting = existingRowsSameCode.find(
      (r) => r.supplierStatus === "Registered" && (r.supplierName || "").trim(),
    );
    const supplierFromExisting = (
      registeredExisting?.supplierName ||
      existingRowsSameCode[0]?.supplierName ||
      ""
    ).trim();
  ...
  const saveSupplierRow = async (idx) => {
    ...
    try {
      ...
      notify(
        "error",
        err.response?.data?.message || "Failed to save supplier row",
      );
    } finally {
  ...
  const handleSupplierCtoChange = (index, field, value) => {
    const supplierStatusByName = getSupplierStatusByName(supplierRows);
    setSupplierCtoRows((prev) => {
      ...
      if (field === "ctoAvailability") {
        const supplierName = (next?.supplierName || "").toString().trim();
        const availability = (value ?? "").toString().trim();
        if (availability === "Not Available") {
          ...
          if (supplierName) {
            ...
            for (let i = 0; i < copy.length; i += 1) {
              const n = (copy[i]?.supplierName || "").toString().trim();
              if (n === supplierName) supplierIndices.push(i);
            }
            let currentCount = supplierIndices.length;
            if (currentCount > 1) {
              for (
                let k = supplierIndices.length - 1;
                k >= 0 && currentCount > 1;
                k -= 1
              ) {
                const idxToRemove = supplierIndices[k];
                if (idxToRemove === index) continue;
                const rowToRemove = copy[idxToRemove];
                if (isSupplierCtoRowEmpty(rowToRemove)) {
          ...
          copy[index] = { ...next, ctoAvailability: "Available" };
        }
      }

      if (field === "ctoPlantNo") {
        const supplierName = (next?.supplierName || "").toString().trim();
        const availability = (next?.ctoAvailability || "Available")
          .toString()
          .trim();
        if (availability === "Not Available") {
          return copy;
        ...
        const raw = (value ?? "").toString().trim();
        const targetCount = parseInt(raw, 10);
        if (supplierName) {
          const desired =
            Number.isFinite(targetCount) && targetCount > 1 ? targetCount : 1;
          ...
          const missing = desired - currentCount;
          if (missing > 0) {
            const insertAt = supplierIndices[supplierIndices.length - 1] + 1;
            const supplierStatus = supplierStatusByName.get(supplierName) || "";
            const newRows = Array.from({ length: missing }, () =>
              createBlankSupplierCtoRow(supplierName, supplierStatus),
            );
            copy.splice(insertAt, 0, ...newRows);
            currentCount += missing;
          } else if (desired < currentCount) {
            for (
              let k = supplierIndices.length - 1;
              k >= 0 && currentCount > desired;
              k -= 1
            ) {
              const idxToRemove = supplierIndices[k];
              if (idxToRemove === index) continue;
      ...
      const currentSupplierName = (copy[index]?.supplierName || "")
        .toString()
        .trim();
      const currentSupplierStatus =
        supplierStatusByName.get(currentSupplierName) || "";
      copy[index] = normalizeSupplierCtoRow(copy[index], currentSupplierStatus);

      return copy;
    });
  };

  const saveSupplierCtoRow = async (idx) => {
    setSavingSupplierCtoRow(idx);
    try {
      const supplierStatusByName = getSupplierStatusByName(supplierRows);
      const cleanedRows = (
        Array.isArray(supplierCtoRows) ? supplierCtoRows : []
      )
        .filter((r) => {
          const supplierName = (r?.supplierName || "").toString().trim();
          if (supplierName) return true;
          return !isSupplierCtoRowEmpty(r);
        })
        .map((row) => {
          const supplierName = (row?.supplierName || "").toString().trim();
          const normalizedRow = normalizeSupplierCtoRow(
            row,
            supplierStatusByName.get(supplierName) || "",
          );
          return {
            ...normalizedRow,
            ctoStartDate: normalizedRow?.ctoStartDate || null,
            ctoValidUpto: normalizedRow?.ctoValidUpto || null,
          };
        });
      const payload = {
        type,
      ...
      const res = await api.post(
        API_ENDPOINTS.CLIENT.SUPPLIER_CTO_CHECK(clientId),
        payload,
      );
      if (res.data.success) {
        const savedRows = mergeSupplierCtoRows(
          getUniqueSupplierNames(supplierRows),
          res.data.data || [],
          supplierStatusByName,
        );
        setSupplierCtoRows(savedRows);
        setLastSavedSupplierCtoRows(savedRows);
  ...
  useEffect(() => {
    const fetchCompliance = async () => {
      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          if (
            item &&
            Array.isArray(item.productComplianceRows) &&
            item.productComplianceRows.length > 0
          ) {
            const fallbackRows = (item.productComplianceRows || []).filter(
              (r) => r && typeof r === "object",
            );
    ...
    const fetchSkuCompliance = async () => {
      ...
      try {
        ...
        if (rows.length) {
          ...
          item &&
          Array.isArray(item.skuComplianceRows) &&
          item.skuComplianceRows.length
        ) {
    ...
    const fetchComponentDetails = async () => {
      ...
      try {
        ...
        if (rows.length) {
          ...
          item &&
          Array.isArray(item.productComponentDetails) &&
          item.productComponentDetails.length
        ) {
      ...
      try {
        ...
        if (rows.length) {
          const normalized = rows.map((r) => ({
            ...r,
            foodGrade: normalizeFoodGrade(r?.foodGrade),
          }));
          ...
          item &&
          Array.isArray(item.productSupplierCompliance) &&
          item.productSupplierCompliance.length
        ) {
          const normalized = item.productSupplierCompliance.map((r) => ({
            ...r,
            foodGrade: normalizeFoodGrade(r?.foodGrade),
          }));
          setSupplierRows(normalized);
    ...
    const fetchSupplierCtoChecks = async () => {
      if (!hasValidProcessSelection) return;
      const supplierNames = getUniqueSupplierNames(supplierRows);
      const supplierStatusByName = getSupplierStatusByName(supplierRows);
      try {
        const res = await api.get(
        ...
        const mergedRows = mergeSupplierCtoRows(
          supplierNames,
          rows,
          supplierStatusByName,
        );
        setSupplierCtoRows(mergedRows);
        setLastSavedSupplierCtoRows(mergedRows);
      } catch (_) {
        const fallbackRows = mergeSupplierCtoRows(
          supplierNames,
          [],
          supplierStatusByName,
        );
        setSupplierCtoRows(fallbackRows);
        setLastSavedSupplierCtoRows(fallbackRows);
      }
    };
    ...
    fetchSupplierCtoChecks();
    fetchRecycledQuantityUsed();
    fetchMonthlyProcurement();
  }, [clientId, type, itemId, item, hasValidProcessSelection]);
  ...
  const steps = [
    {
      id: "verification",
      label: "Verification",
      description: "CTE & CTO/CCA Details",
    },
    {
      id: "tab2",
      label: "Product Compliance",
      description: "Packaging and polymer details",
    },
    {
      id: "procurement",
      label: "Single Use Plastic",
      description: "Supplier & Single Use Plastic Details",
    },
    {
      id: "tab5",
      label: "Summery Report",
      description: "Overall compliance summary report",
    },
  ];
  ...
  const subSteps = [
    ...
    { id: "supplier-cto-check", label: "Supplier CTO Check" },
  ];
  ...
  const fetchSupChecklist = useCallback(async () => {
    if (!clientId || !type || !itemId) return;
    try {
      const res = await api.get(API_ENDPOINTS.CLIENT.SUP_CHECKLIST(clientId), {
        params: { type, itemId },
      });
      if (res.data.success) {
  ...
  const saveSupChecklist = useCallback(
    async (rows, showNotification = true) => {
      try {
        const res = await api.post(
          API_ENDPOINTS.CLIENT.SUP_CHECKLIST(clientId),
          {
            type,
            itemId,
            rows,
  ...
  const fetchProcurement = useCallback(async () => {
    if (!clientId || !type || !itemId) return;
    try {
      const res = await api.get(API_ENDPOINTS.CLIENT.PROCUREMENT(clientId), {
        params: { type, itemId },
      });
      if (res.data.success) {
  ...
  const handleProcurementUpload = async (file) => {
    ...
    try {
      const res = await api.post(
        API_ENDPOINTS.CLIENT.PROCUREMENT(clientId),
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
  ...
  useEffect(() => {
    if (item) {
      if (item.completedSteps && Array.isArray(item.completedSteps)) {
        ...
        item.completedSteps &&
        typeof item.completedSteps === "string"
      ) {
        try {
  ...
  useEffect(() => {
    if (client && item) {
      ...
      const allCte = (client.productionFacility?.cteDetailsList || []).map(
        (i) => ({ ...i, type: "CTE" }),
      );
      const allCto = (client.productionFacility?.ctoDetailsList || []).map(
        (i) => ({ ...i, type: "CTO" }),
      );

      const matches = [...allCte, ...allCto].filter(
        (i) => normalize(i.plantName) === currentPlantName,
      );
  ...
  const handleSaveSummary = async () => {
    ...
    try {
      ...
      const uploadPromises = Array.from(uniqueUploads.values()).map(
        async ({ index, row }) => {
          const formData = new FormData();
          formData.append("type", type);
          formData.append("itemId", itemId);
          ...
          try {
            ...
            if (res.data.success && res.data.data && res.data.data.row) {
              return {
                skuCode: row.skuCode,
                url: res.data.data.row.additionalDocument,
              };
            }
          } catch (err) {
      ...
      const payload = updatedRows.map((r) => {
        ...
        return {
          ...
          componentImage:
            typeof rest.componentImage === "string" ? rest.componentImage : "",
          additionalDocument:
            typeof rest.additionalDocument === "string"
              ? rest.additionalDocument
              : "",
        };
      });
  ...
  const getChangeSummary = () => {
    ...
    productRows.forEach((row, idx) => {
      ...
      fields.forEach((field) => {
        ...
        if (
          getComparableProductValue(initialVal, field) !==
          getComparableProductValue(currentVal, field)
        ) {
          changes.push({
  ...
  const historyModalData = combinedHistory.length
    ? combinedHistory
  ...
  useEffect(() => {
    setLoading(clientQuery.isLoading);
  }, [clientQuery.isLoading]);

  useEffect(() => {
    if (clientQuery.data) {
      setClient(clientQuery.data);
      setError("");
    }
  }, [clientQuery.data]);
  ...
  useEffect(() => {
    if (clientQuery.error) {
      setError(
        clientQuery.error?.response?.data?.message ||
  ...
  const findItem = () => {
    ...
    let foundItem = null;
    if (type === "CTE") {
      foundItem = client.productionFacility.cteDetailsList?.find(
        (i) => i._id === itemId,
      );
    } else if (type === "CTO") {
      foundItem = client.productionFacility.ctoDetailsList?.find(
        (i) => i._id === itemId,
      );
    }
  ...
  const handleVerify = async (status, targetItem) => {
    ...
    try {
      const response = await api.post(
        API_ENDPOINTS.CLIENT.VERIFY_FACILITY(clientId),
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
  ...
  const indexOfFirstComponentRow =
    indexOfLastComponentRow - componentItemsPerPage;
  const currentComponentRows = componentRows.slice(
    indexOfFirstComponentRow,
    indexOfLastComponentRow,
  );
  const totalComponentPages = Math.ceil(
    componentRows.length / componentItemsPerPage,
  );
  ...
  const currentSupplierRows = supplierRows.slice(
    indexOfFirstSupplierRow,
    indexOfLastSupplierRow,
  );
  const totalSupplierPages = Math.ceil(
    supplierRows.length / supplierItemsPerPage,
  );
  ...
  const currentRecycledRows = recycledRows.slice(
  ...
  const totalRecycledPages = Math.ceil(
        ...
        {activeTab === "tab2" && (
          <SupplierCompliance>
            ...
            <ProductCompliance
              ...
              supplierCtoRows={supplierCtoRows}
              handleSupplierCtoChange={handleSupplierCtoChange}
              saveSupplierCtoRow={saveSupplierCtoRow}
              savingSupplierCtoRow={savingSupplierCtoRow}
            ...
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  width: 250,
                  render: (text) => (
                    <div className="max-w-[250px] break-all whitespace-pre-wrap line-through text-gray-500 text-xs decoration-red-400">
                      {text}
                    </div>
                  ),
                },
                {
                  ...
                  width: 250,
                  render: (text) => (
                    <div className="max-w-[250px] break-all whitespace-pre-wrap font-bold text-primary-700 text-xs">
                      {text}
                    </div>
                  ),
                },
